...
public abstract class DragonCacheBuilder {
	public static <K, V> DragonCache<K, V> build(int initialCapacity, int maximumSize, int concurrencyLevel,
	      int expireAfterAccess, TimeUnit timeUnit) {
		Cache<K, V> cache = CacheBuilder.newBuilder().initialCapacity(initialCapacity)// 初始容量，最多可以自动扩容到maximumSize
