...
public class SQLStatementParser extends SQLParser {
    ...
    public void parseStatementList(List<SQLStatement> statementList, int max) {
        for (;;) {
            if (max != -1) {
                if (statementList.size() >= max) {//解析出来的sql语句个数，已经大于指定的个数
                    return;
                }
            }
            ...
            if (lexer.token() == Token.SELECT) {//解析SQL查询，解析结果放入statementList中
                statementList.add(parseSelect());
                continue;
            }

            if (lexer.token() == (Token.UPDATE)) {//解析SQL更新，解析结果放入statementList中
                statementList.add(parseUpdateStatement());
                continue;
            }

            if (lexer.token() == (Token.CREATE)) {//解析SQL创建，解析结果放入statementList中
