...
public class Lexer {
    ...
    protected final String text;//待分析的字符串
    protected int          pos; //当前扫描的字符 在字符串中的位置
    protected int          mark;//词素的开头

    protected char         ch;//当前扫描的字符   ch=text.charAt(pos)

    protected char[]       buf;
    protected int          bufPos;//存放找的词素的长度
    ...
    public final void nextToken() {
        ...
        this.lines = 0;
        int startLine = line;//从当前解析到的行，开始解析
        ...
        for (;;) {
            ...
            if (ch == '$' && charAt(pos + 1) == '{') {//向前探测一位，才能确定应该返回哪个词法单元
                scanVariable();
                return;
            }
            ...
            switch (ch) {
                case '0':
                    if (charAt(pos + 1) == 'x') {
                        scanChar();
                        scanChar();
                        scanHexaDecimal();//如果是0x开头，可能是需要扫描16禁止数
                    } else {
                        scanNumber();
                    }
                    return;
